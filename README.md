...
[Conformal prediction](http://www.alrw.net/articles/03.pdf) is a mathematical framework that allows to assign a confidence level to each prediction, basing on a solid background theory. This contrasts to current best-practices (e.g. [cross-validation](https://en.wikipedia.org/wiki/Cross-validation_%28statistics%29)) where an overall confidence level for predictions on new examples is hedged basing on previous performance. Given a user-specified significance level 𝜺, instead of producing a single prediction, a **conformal predictor** outputs a **prediction set** of outcomes. The major advantage of this approach is that, within the mathematical framework, there is proof that *the true label for a new object will be in the prediction set with probability 1 - 𝜺*.