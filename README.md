...
In pharmaceutical bioinformatics, assigning a confidence level to predictions plays a major role. In fact, if you think to the toxicology models that we built in the previous examples, due to security reasons, the prediction will be useful in practice only if we can assign to them a valid confidence, that is the likelihood for the prediction to be correct. 
