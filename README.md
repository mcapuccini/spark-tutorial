...
We say that a conformal predictor is **valid** when, for a certain significance level  𝜺, the observed error rate will be at most 𝜺. This is always true **on average**, by construction. However, a conformal predictor that outputs too big prediction sets is unuseful. In fact, if we recall the toxicology example we are just interested in **singleton prediction sets**; empty or both classes *{toxic, non-toxic}* prediction sets means that, at the user-specified significance level, the prediction is *rejected*. 

In [binary classification](https://en.wikipedia.org/wiki/Binary_classification) (e.g. toxicology predictive modelling), we define the **efficiency** of a conformal prediction, with respect to a certain significance level, as the observed *singleton prediction set rate*. This measure tells us how useful a conformal predictior is for certain significance level. 