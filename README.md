...
We say that a conformal predictor is **valid** when, for a certain significance level  𝜺, the observed error rate is at most 𝜺. This is always true **on average**, by construction. However, a conformal predictor that outputs too big prediction sets is unuseful. In fact, if we recall the toxicology example we are just interested in **singleton prediction sets**; empty or both classes *{toxic, non-toxic}* prediction sets means that, at the user-specified significance level, the prediction is *rejected*. 
