...
The main data abstraction in Spark programs is the [Resilient Distributed Dataset](http://spark.apache.org/docs/latest/programming-guide.html#resilient-distributed-datasets-rdds) (RDD), which is a distributed collection of object that can be transformed, via built-in functions, in parallel. The main properties of a RDD are: resilience (or elasticity), fault-tolerance and cacheability. The last property is of particular interest for iterative tasks such as machine learning algorithms. In the following code snippet, we use an RDD, along with its built-in function, to implement a Spark-based parallel *word count* program. Since we are using RDDs, such implementation will be out-of-the-box resilient and fault tolerant.  
